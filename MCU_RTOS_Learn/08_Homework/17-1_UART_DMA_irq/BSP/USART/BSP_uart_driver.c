...
void UartDriverFunc(void *argument)
{
    ...
    // 环形缓冲区空间挂载
    log_d(" g_circular_buf_irq_thread = [0x%p] ", g_circular_buf_irq_thread ) ;
    ...
    for(;;)
    {
        ...
        if( IRQ_SEND_TO_THREAD == receive_data )
        {
            ...
            {
                // log_i("  xQueueSendToBack is Send to end Success ! \n");
            }
        }
...
/**
 * @brief  串口 DMA 传输 环形缓冲区半满中断 回调函数
 * 
...
{
    log_d(" dma_halffull_irq : [%d] ", number_of_data);
    // 获取当前位置
    uint32_t head_pos = 0 ;
    uint8_t ret = 0 ;

    ret = Cirbuf_GetHeadPos(g_circular_buf_irq_thread, &head_pos);
    if( 0x00 != ret )
    {
        log_e(" Get head position error " );
    }
    log_d(" head_pos : [%d] ", head_pos);
    // 获取进入半满中断时，数据已经到达的位置（ CIRCULAR_BUFFER_SIZE / 2 - 1 ） 缓冲区长度一半，半满中断的长度
    uint32_t current_data_pos = ( CIRCULAR_BUFFER_SIZE / 2 )  ;         // ERROR ：( CIRCULAR_BUFFER_SIZE / 2 ) - 1
    log_i(" current_data_pos : [%d] ", current_data_pos);
 
    // 对 head 进行取余数
    uint32_t pos_in_buffer = head_pos%( CIRCULAR_BUFFER_SIZE / 2 ) ;
    log_i(" pos_in_buffer : [%d] ", pos_in_buffer);

    // 算出当前应该偏移的数量
    // uint32_t move_pos = current_data_pos - pos_in_buffer ;
    // log_i(" move_pos : [%d] ", move_pos);
    uint32_t move_pos = 0 ;
    if(current_data_pos < pos_in_buffer )       // BUG 修复：当全满时会出现负数，导致不会头指针不会递增
    {
        move_pos = (current_data_pos+CIRCULAR_BUFFER_SIZE) - pos_in_buffer ;
        log_i(" move_pos complete : [%d] ", move_pos);
    }else
    {
        move_pos = current_data_pos - pos_in_buffer ;
        log_i(" move_pos normal : [%d] ", move_pos);
    }

    // 对当前位置进行累加
    Cirbuf_HeadPositionIncrement( g_circular_buf_irq_thread, move_pos);
    ...
    //     log_e(" Get head position error " );
    // }
    // log_d(" move head_pos : [%d] ", head_pos);

    // 对前端线程进行通知
    // TODO
    uint32_t send_to_thread = IRQ_SEND_TO_THREAD ;
    BaseType_t ret_queue = xQueueGenericSendFromISR(g_queue_uart_irq_thread, &send_to_thread, NULL, queueOVERWRITE);
    if( pdTRUE != ret_queue )
    {
        log_d(" BSP_DMA_HalfFull_Irq_Callback  is Send to front FAIL ! \n");
    }

    // uint32_t end_pos_in_buffer = head_pos%( CIRCULAR_BUFFER_SIZE / 2 ) ;
    // log_i(" pos_in_buffer : [%d] ", end_pos_in_buffer);

}


/**
 * @brief  串口 DMA 传输 环形缓冲区全满中断 回调函数
 * 
 * @param [in]  number_of_data 
 * 
 * @details
...
{
    log_d(" dma_Completefull_irq : [%d] ", number_of_data);
    ...
    // 获取进入半满中断时，数据已经到达的位置（ CIRCULAR_BUFFER_SIZE  ）
    uint32_t current_data_pos = ( CIRCULAR_BUFFER_SIZE  )  ;        // ERROR ：( CIRCULAR_BUFFER_SIZE  ) - 1
    log_i(" C:current_data_pos : [%d] ", current_data_pos);
    ...
    // 算出当前应该偏移的数量
    // uint32_t move_pos = current_data_pos - pos_in_buffer ;
    // log_i(" C: move_pos : [%d] ", move_pos);
    uint32_t move_pos = 0 ;
    if(current_data_pos < pos_in_buffer )       // BUG 修复：当全满时会出现负数，导致不会头指针不会递增
    {
        move_pos = (current_data_pos+CIRCULAR_BUFFER_SIZE) - pos_in_buffer ;
        log_i(" C: move_pos complete : [%d] ", move_pos);
    }else
    {
        move_pos = current_data_pos - pos_in_buffer ;
        log_i(" C: move_pos normal : [%d] ", move_pos);
    }
    ...
    //     log_e(" Get head position error " );
    // }
    // log_d(" C:move head_pos : [%d] ", head_pos);
    ...
    if( pdTRUE != ret_queue )
    {
        log_d(" BSP_DMA_CompleteFull_Irq_Callback xQueueSendToBackFromISR is Send to front FAIL ! \n");
    }

}

/**
 * @brief  串口 DMA 传输 环形缓冲区 串口空闲 回调函数
 * 
 * @param [in]  number_of_data 
 * 
 * @details
...
void BSP_UART_Idle_Irq_Callback( uint32_t number_of_data )
{
    ...
    // 获取进入半满中断时，数据已经到达的位置 number_of_data -1
    uint32_t current_data_pos = number_of_data ;                 // ERROR ：number_of_data -1
    log_i(" U:current_data_pos : [%d] ", current_data_pos);
 
    // 对 head 进行取余数
    uint32_t pos_in_buffer = head_pos%( CIRCULAR_BUFFER_SIZE  ) ;
    log_i(" U:pos_in_buffer : [%d] ", pos_in_buffer);
    ...
    {
        move_pos = (current_data_pos+CIRCULAR_BUFFER_SIZE) - pos_in_buffer ;
        log_i(" U: move_pos complete : [%d] ", move_pos);
    }else
    {
        move_pos = current_data_pos - pos_in_buffer ;
        log_i(" U: move_pos normal : [%d] ", move_pos);
    }
    ...
    //     log_e(" Get head position error " );
    // }
    // log_d(" U:move head_pos : [%d] ", head_pos);
    ...
    if( pdTRUE != ret_queue )
    {
        log_d("  BSP_UART_Idle_Irq_Callback xQueueSendToBackFromISR is Send to front FAIL ! \n");
    }
