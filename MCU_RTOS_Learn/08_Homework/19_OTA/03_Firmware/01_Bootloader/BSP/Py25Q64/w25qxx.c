...
uint8_t W25Qx_Get_Parameter(W25Qx_Parameter *Para)
{
	...
	W25Qx_Read_ID(&id);
	//if(id < W25Q80 || id > W25Q128) return W25Qx_ERROR;     // 需要修改
    if(id != 0x8516)    return W25Qx_ERROR ;
	
	size = (uint32_t) powf(2,(id - 0xEF13)) * 1024 * 1024;
	
	Para->FLASH_ID = id;
	Para->FLASH_SIZE = 0x7FFFFF;    //  size;
...
uint8_t W25Qx_Read(uint8_t* pData, uint32_t ReadAddr, uint32_t Size)
{
	...
	SPI1_WriteByte( cmd, 4, W25QXXXX_TIMEOUT_VALUE);	
	/* Reception of the data */
	if (SPI1_ReadByte( pData,Size,W25QXXXX_TIMEOUT_VALUE) != 0x01)
  {
    return W25Qx_ERROR;
  }
	W25Qx_Disable();
...
uint8_t W25Qx_Write(uint8_t* pData, uint32_t WriteAddr, uint32_t Size)
{
  ...
  do
  {
		...
		W25Qx_Enable();
    /* Send the command */
    if (SPI1_WriteByte(cmd, 4, W25QXXXX_TIMEOUT_VALUE) != 0x01)
    {
      return W25Qx_ERROR;
    }
    
    /* Transmission of the data */
    if (SPI1_WriteByte( pData,current_size, W25QXXXX_TIMEOUT_VALUE) != 0x01)
    {
      return W25Qx_ERROR;
    }
		W25Qx_Disable();
